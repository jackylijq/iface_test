...
from tools import mg_db_manager,usefulTools,switch,db_manager,utils_logging
...
class case_excute():
    ...
    def api_request_entry(self, case_request):
        ...
        for case_id in range(len(test_case_list)):
            ...
            if 'product_name' not in test_case_keys:
                test_result_infor = self.standard_case_excute(test_case_list[case_id])
    ...
    #标准api请求入口
    def standard_case_excute(self,case_infor):
        '''
        1、定义结果参数，或许会作为全局参数
        ...
        '''
        #组装请求参数：
        case_id = u'当前测试用例id：%s' %case_infor['case_id']
        ...
        #获取需要校验的key列表
        compare_key_list = test_case_infor['compare_key_list']
...
if __name__ == '__main__':
    standard_api_instances = case_excute()
