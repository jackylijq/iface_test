...
sql_modle = 'CREATE TABLE `%s` (`id` int(0) NOT NULL AUTO_INCREMENT, %s , PRIMARY KEY (`id`) USING BTREE);'
...
class db_handle():
    ...
    def generate_table_structure(self):
        field_one_int = '`%s` int(0) NULL DEFAULT NULL'
        field_one_varchar = '`%s` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL'
        field_one_time = '`%s` datetime(0) NULL DEFAULT NULL'
        table_field_list = []
        #增加int类型数据
        for i in range(9):
    ...
    def insert_date_batch(self):
        for i in range(500):
            ...
            table_field_infor = self.generate_table_structure()
...
if __name__ == '__main__':
    ...
    # create_file_instances.thread_batch()