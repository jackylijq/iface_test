...
class db_handle():
    ...
    def get_conn(self):
        ...
        db = db_config["host"] + ":" + str(db_config["port"]) + "/" + db_config["db"]
    ...
    def insert_data_db(self,table_name,args):
        basic_sql = 'INSERT INTO "%(db_name)s"."%(table_name)s" VALUES ('
        # basic_sql = 'INSERT INTO "public"."%(table_name)s" VALUES (0,'
        basic_param = {'db_name':db_config["user_db"],'table_name':table_name}
        for i in range(len(args)):
            sql_last = ',' if i < len(args)-1 else ''
            if type(args[i]) == int or args[i] == 'sysdate':
    ...
    def make_data(self,int_num,str_num):
        ...
        data_list.append('sysdate')
        return data_list
    #生成库表结构
    def generate_table_structure(self,int_num,str_num):
        field_one_int = '"%s" NUMBER NOT NULL'
        field_one_varchar = '"%s" VARCHAR2(255 BYTE)'
    ...
    def insert_date_batch(self,int_num,str_num,cycle_num):
        ...
        for i in range(cycle_num):
            ...
            generate_table_sql = sql_modle % (db_config['user_db'],table_name, table_field_infor)
            # table_name_list.append(generate_table_sql)
            # self.data_insert_update(table_name_list)
            #建表语句加入到sql_list
            sql_list.append(generate_table_sql)
            # 先生成基础数据
    ...
    def thread_batch(self, table_num, field_num, supervene_num=200):
        ...
        l_thread = (Thread(target=self.insert_date_batch, args=(int_num, str_num, cycle_num)) for i in
                    range(supervene_num))
        for t in l_thread:
            t.start()  # 启动线程开始执行
...
if __name__ == '__main__':
    ...
    create_file_instances.insert_date_batch(1,2,1)
    # create_file_instances.data_insert_update('-')
    # create_file_instances.thread_batch(10000,500)